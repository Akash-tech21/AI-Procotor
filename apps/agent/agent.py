...
class TutorScreener(Agent):
    ...
    def __init__(self, interview_id: str, candidate_name: str | None = None) -> None:
        super().__init__(instructions=INTERVIEWER_SYSTEM_PROMPT)
        self.interview_id = interview_id
        self.state = InterviewState(candidate_name=candidate_name)
        ...
        try:
            while self.state.phase != Phase.ENDED:
                ...
                if (
                    ...
                    nudge = build_silence_nudge(self.state)
                    self.session.generate_reply(instructions=nudge)
...
async def entrypoint(ctx: agents.JobContext) -> None:
    ...
    # Extract the candidate's display name from the first remote participant.
    ...
    agent = TutorScreener(interview_id=interview_id, candidate_name=candidate_name)
